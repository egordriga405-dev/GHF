...
<body>
    ...
    <script>
        ...
        function renderTopics() {
            ...
            container.innerHTML = examData.map(topic => {
                ...
                return '<div class="topic-chip' + (isActive ? ' active' : '') + 
        ...
        function renderQuestions() {
            ...
            container.innerHTML = topic.questions.map((q, i) => {
                return '<div class="question-card">' +
                    ...
                    '</div>' +
                    '<button class="btn" onclick="toggleAnswer(\\'' + q.id + '\\')">👁️ Показать ответ</button>' +
                    '</div>';
            }).join('');
        }
        ...
        function hideAllAnswers() {
            ...
            document.querySelectorAll('.question-card .btn').forEach(b => {
                b.textContent = '👁️ Показать ответ';
            });
        }
    ...
    async shareFile() {
        try {
            ...
            const file = new File([blob], 'exam-trainer.html', { type: 'text/html' });

            if (navigator.share && navigator.canShare && navigator.canShare({ files: [file] })) {
                await navigator.share({
                    files: [file],
                    title: 'Тренажёр для экзаменов',
                    text: 'Откройте файл в браузере для тренировки!'
                });
                return true;
            }
            return false;
